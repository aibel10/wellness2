...
export default function SuccessStoriesPage() {
  ...
  return (
    <main className="min-h-screen relative overflow-hidden bg-page text-ink-slate transition-colors duration-300">
      ...
      <div className="relative z-10 flex flex-col flex-1 pt-24 md:pt-32 pb-16 md:pb-24">
        ...
        <section className="mx-auto max-w-4xl px-6 mb-16 md:mb-24 text-center">
          ...
          <p className="text-lg text-ink-slate max-w-2xl mx-auto font-medium">
            ...
            {stories.map((story, index) => (
              ...
              >
                ...
                <div className="w-full md:w-1/2">
                  <div className="relative aspect-square sm:aspect-square md:aspect-square lg:aspect-[4/3] rounded-[2rem] overflow-hidden shadow-lg border border-border-theme bg-bg-card backdrop-blur-md group">
                    <Image 
                      src={story.img} 
                ...
                <div className="w-full md:w-1/2 flex flex-col justify-center">
                  ...
                  <h3 className="text-3xl md:text-4xl font-bold text-ink mb-2">
                    {story.name}
                  </h3>
                  <p className="text-xs font-bold uppercase text-moss tracking-widest mb-6">
                    {story.category}
                  </p>
                  <div className="bg-bg-card-hover backdrop-blur-xl border border-border-theme p-6 md:p-8 rounded-[1.5rem] relative shadow-md transition-all hover:bg-mist/90 hover:border-moss/30">
                    <Quote className="absolute top-6 left-6 w-8 h-8 text-moss/20" />
                    <p className="text-ink-slate text-lg font-semibold leading-relaxed italic relative z-10 pl-8">
                      &quot;{story.quote}&quot;
                    </p>
                  </div>
                </div>
        ...
        <section className="mx-auto max-w-4xl px-6 mt-24 text-center">
           <div className="bg-bg-card border border-border-theme rounded-[2rem] p-10 md:p-16 text-ink-slate shadow-md relative overflow-hidden">
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold mb-6 text-ink relative z-10">Ready for Your Own Transformation?</h2>
              <p className="text-ink-slate text-lg mb-8 relative z-10 font-semibold">Join the Eat Right community today and take the first step to sustainable wellness.</p>
...
function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-border-theme bg-bg-header text-ink backdrop-blur-xl shadow-soft">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Main navigation" className="hidden items-center gap-7 md:flex">
          {navItems.map((item) => (
            <a
              className="focus-ring rounded-full px-4 py-2 text-sm font-semibold text-ink-slate transition-all duration-300 border border-transparent hover:text-ink hover:bg-bg-button hover:backdrop-blur-md hover:border-border-theme"
              href={item.href}
              key={item.href}
            >
              {item.label}
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <a
            className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-moss px-4 text-sm font-bold text-white shadow-md transition hover:bg-ink hover:text-page"
          ...
          <div className="space-y-6">
            <a href="/" className="inline-block">
              <h2 className="text-2xl font-bold tracking-tight">
                <span className="text-ink">ASUHAR B</span>

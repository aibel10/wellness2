...
export default function SuccessStoriesPage() {
  ...
  return (
    <main className="min-h-screen relative overflow-hidden bg-[#060B15] text-white">
      ...
      <div className="relative z-10 flex flex-col flex-1 pt-24 md:pt-32 pb-16 md:pb-24">
            ...
            {stories.map((story, index) => (
              ...
              >
                ...
                <div className="w-full md:w-1/2">
                  <div className="relative aspect-square sm:aspect-square md:aspect-square lg:aspect-[4/3] rounded-[2rem] overflow-hidden shadow-2xl border border-white/10 bg-mist/30 backdrop-blur-md group">
                    <Image 
                      src={story.img} 
                ...
                <div className="w-full md:w-1/2 flex flex-col justify-center">
                  ...
                  <div className="bg-mist/60 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-[1.5rem] relative shadow-2xl transition-all hover:bg-mist/80 hover:border-moss/30">
                    <Quote className="absolute top-6 left-6 w-8 h-8 text-moss/20" />
                    <p className="text-white/80 text-lg font-medium leading-relaxed italic relative z-10 pl-8">
                      &quot;{story.quote}&quot;
                    </p>
                  </div>
                </div>
        ...
        <section className="mx-auto max-w-4xl px-6 mt-24 text-center">
           <div className="bg-mist border border-white/10 rounded-[2rem] p-10 md:p-16 text-white shadow-2xl relative overflow-hidden">
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold mb-6 relative z-10">Ready for Your Own Transformation?</h2>
              <p className="text-white/70 text-lg mb-8 relative z-10 font-semibold">Join the Eat Right community today and take the first step to sustainable wellness.</p>
              <a href="https://wa.me/919809745714?text=Hi%20Asuhar%20B,%20I'm%20interested%20in%20joining%20the%20wellness%20program." target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 items-center justify-center rounded-xl bg-moss px-8 font-bold text-white shadow-lg shadow-moss/20 transition-all hover:scale-105 hover:shadow-xl relative z-10">
...
function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#060B15]/80 text-white backdrop-blur-xl shadow-soft">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Main navigation" className="hidden items-center gap-7 md:flex">
          {navItems.map((item) => (
            <a
              className="focus-ring rounded-full px-4 py-2 text-sm font-semibold text-white/95 transition-all duration-300 border border-transparent hover:text-white hover:bg-white/10 hover:backdrop-blur-md hover:border-white/20"
              href={item.href}
              key={item.href}
            >
              {item.label}
        ...
        <div className="flex items-center gap-2">
          <a
            className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-moss px-4 text-sm font-bold text-white shadow-md transition hover:bg-white hover:text-black"
